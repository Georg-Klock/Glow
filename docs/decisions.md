...
generated catalogue now carries a spoken name for every offered tile. Symbols
matched from the emoji catalogue preserve that entry’s human Unicode name;
the additional fitness figures receive short activity descriptions generated
beside the list. VoiceOver therefore says “Traditional strength training”
instead of “figure strengthtraining traditional.”

The speech mapping is regenerated by `Tools/make-symbol-catalog.py` with the
visible catalogue. A test requires every browsable symbol to have a nonempty,
non-identifier description, so a future Xcode refresh cannot silently add an
unlabelled or technically labelled tile.
## 2026-09-02 — An open widget span has one control per eligible day (#523)

WidgetKit reports activation, not a coordinate inside a custom control. One
toggle around an open Monday–Wednesday span could therefore only write its
fallback Wednesday even when Monday was pressed. The installed widget and the
Widgets-tab preview now keep one continuous socket but overlay one exact-date
`SlotToggle` per nonfuture, non-rest column. The visual preview remains a true
preview of the Home Screen behavior.

The socket is still drawn once. Clear off-faces avoid repeating its expensive
inner-shadow stack and preserve scrolling performance; the tapped control's
optimistic on-face overlays a lit day-sized mark immediately, then the normal
reload replaces it with the row's authoritative redivision. Filled spans keep
