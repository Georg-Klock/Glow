...
enum GoalPop {
    /// Everything the pop can say. 218 phrases, one pool — Georg's final cut
    /// of 2026-09-07, read from his list in his order.
    ///
    /// **Nine varieties of English, on purpose** (#471), then pruned by ear.
    /// The list opened as one register — themed and a little arch, with plain
    /// speech a tail of about seventeen — so a draw rarely landed on anything a
    /// person would actually say out loud. #471 widened it to 370 across plain
    /// American, British, Irish, Australian, New Zealand, Scottish, Welsh,
    /// South African, Indian and Caribbean English. The final cut keeps the
    /// themed run that earned its place and the plain speech, with the
    /// regional voices that read as a voice rather than a costume: `proper
    /// job`, `grand stuff`, `good onya`, `away ye go`, `there's lovely`,
    /// `bless up`. The naturalised loanwords — `ka pai`, `shabash`, `kya
    /// baat`, `lekker`, `aweh`, `irie` — went with the cut, along with the
    /// long tail of near-duplicates the widening carried.
    ///
    ...
    static let lines = [
        // The themed run: sport, work, games, the odd flourish.
        "yes slay",
        "ate that",
        "no notes",
        "certified icon",
        ...
        // Plain speech, and the regional voices that survived the final cut.
        "encore please",
        "wonderful",
        "good move",
        "miles logged",
